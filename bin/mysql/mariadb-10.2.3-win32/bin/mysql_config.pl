...
#        handle -lmysqlclient    and other strings inserted that might contain
...
$flags->{libs} =
  [@ldflags,@lib_opts,'','','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'','-lmysqlclient   ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','','-lmysqlclient   ','',''];
